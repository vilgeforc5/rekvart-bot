...
import {
  zamerApi,
  type CreateZamerQuestionDto,
  type UpdateZamerQuestionDto,
  type ZamerQuestion,
  type ZamerVariant,
} from "../lib/zamer.api";
...
export function Zamer() {
  ...
  const createQuestionMutation = useMutation({
    mutationFn: (data: CreateZamerQuestionDto) => zamerApi.createQuestion(data),
    onSuccess: () => {
  ...
  const deleteQuestionMutation = useMutation({
    mutationFn: (id: number) => zamerApi.deleteQuestion(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["zamer-config"] });
    },
  });
  ...
  const onSubmitQuestion = (formData: QuestionFormData) => {
    ...
    const questionData: CreateZamerQuestionDto | UpdateZamerQuestionDto = {
      text: formData.text,
      type: formData.type,
    ...
    if (editingQuestionId !== null) {
      updateQuestionMutation.mutate({
        ...
        data: questionData as UpdateZamerQuestionDto,
      });
    } else {
      createQuestionMutation.mutate(questionData as CreateZamerQuestionDto);
    }
  };

  const handleEditQuestion = (question: ZamerQuestion) => {
  ...
  const handleDragEndQuestions = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      queryClient.setQueryData(["zamer-config"], {
        ...config,
      ...
      try {
        await Promise.all(
          reorderedQuestions.map((q, index) =>
            zamerApi.updateQuestion(q.id, { order: index + 1 })
          )
        );
      } catch {
        queryClient.invalidateQueries({ queryKey: ["zamer-config"] });
  ...
  const handleDragEndVariants = async (
    ...
    if (over && active.id !== over.id) {
      ...
      try {
        await zamerApi.updateQuestion(questionId, {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <h3 className="text-sm text-gray-500 font-semibold mb-6">
        Настройка вопросов и итогового сообщения для команды /zamer
      </h3>
...
function SortableQuestionItem({
  ...
  question: ZamerQuestion;
  onEdit: (question: ZamerQuestion) => void;
...
function SortableVariantItem({ variant }: { variant: ZamerVariant }) {
  const { attributes, listeners, setNodeRef, transform, transition } =
