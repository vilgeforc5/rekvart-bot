...
import {
  consultacyaApi,
  type ConsultacyaQuestion,
  type ConsultacyaVariant,
  type CreateConsultacyaQuestionDto,
...
export function Consultacya() {
  ...
  const createQuestionMutation = useMutation({
    mutationFn: (data: CreateConsultacyaQuestionDto) =>
      consultacyaApi.createQuestion(data),
    onSuccess: () => {
  ...
  const deleteQuestionMutation = useMutation({
    mutationFn: (id: number) => consultacyaApi.deleteQuestion(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["consultacya-config"] });
    },
  });
  ...
  const onSubmitQuestion = (formData: QuestionFormData) => {
    ...
    const questionData:
      ...
      variants: editingVariants.map((v, idx) => ({
        id: v.id,
        text: v.text,
        order: idx + 1,
    ...
    if (editingQuestionId !== null) {
      updateQuestionMutation.mutate({
        ...
        data: questionData as UpdateConsultacyaQuestionDto,
      });
    } else {
      createQuestionMutation.mutate(
        questionData as CreateConsultacyaQuestionDto
      );
    }
  };

  const handleEditQuestion = (question: ConsultacyaQuestion) => {
  ...
  const handleDragEndQuestions = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      queryClient.setQueryData(["consultacya-config"], {
        ...config,
      ...
      try {
        await Promise.all(
          reorderedQuestions.map((q, index) =>
            consultacyaApi.updateQuestion(q.id, { order: index + 1 })
          )
        );
      } catch {
        queryClient.invalidateQueries({ queryKey: ["consultacya-config"] });
  ...
  const handleDragEndVariants = async (
    ...
    if (over && active.id !== over.id) {
      ...
      try {
        await consultacyaApi.updateQuestion(questionId, {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h2 className="text-2xl font-bold text-gray-900 mb-2">Консультация</h2>
      <h3 className="text-sm text-gray-500 font-semibold mb-6">
        Настройка вопросов и итогового сообщения для команды /consultacya
      </h3>
          ...
          >
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Введите итоговое сообщение после прохождения консультации"
              />
            </div>
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <label className="flex items-center gap-2 text-sm text-gray-700">
                        <input
                          ...
                          onChange={(e) =>
                            updateVariant(index, "needsPhone", e.target.checked)
                          }
...
function SortableQuestionItem({
  ...
  question: ConsultacyaQuestion;
  onEdit: (question: ConsultacyaQuestion) => void;
...
function SortableVariantItem({ variant }: { variant: ConsultacyaVariant }) {
  const { attributes, listeners, setNodeRef, transform, transition } =
