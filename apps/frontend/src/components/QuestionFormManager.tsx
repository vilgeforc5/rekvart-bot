...
interface QuestionFormManagerProps {
  ...
  onUpdateSummary: (message: string) => Promise<any>;
...
export function QuestionFormManager({
  ...
  const onSubmitQuestion = (formData: QuestionFormData) => {
    if (editingQuestionId !== null) {
      ...
      onUpdateQuestion(editingQuestionId, questionData).then(() => {
        resetQuestion();
        setEditingVariants([]);
    ...
    if (over && active.id !== over.id && config) {
      ...
      await Promise.all(
        reorderedQuestions.map((q, index) =>
          onUpdateQuestion(q.id, { order: index + 1 })
        )
      );
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <h3 className="text-sm text-gray-500 font-semibold mb-6">
        Настройка вопросов и итогового сообщения для команды {commandName}
      </h3>
        ...
        >
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={`Введите итоговое сообщение после прохождения ${title.toLowerCase()}`}
            />
          </div>
      ...
      {editingQuestionId !== null && (
        <div className="mb-8">
          <form
            ...
            className="bg-white shadow rounded-lg p-6 space-y-4 max-w-2xl ring-2 ring-blue-500"
          >
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
